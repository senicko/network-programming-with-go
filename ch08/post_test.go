...
func TestPostUser(t *testing.T) {
	...
	if resp.StatusCode != http.StatusMethodNotAllowed {
		t.Fatalf("expexted status %d; actual status %d", http.StatusMethodNotAllowed, resp.StatusCode)
	}
...
func TestMultipartPost(t *testing.T) {
	...
	for i, file := range []string{
		...
		filePart, err := w.CreateFormFile(fmt.Sprintf("field%d", i+1), filepath.Base(file))
		if err != nil {
			t.Fatal(err)
		}
