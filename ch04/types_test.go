...
func TestPayloads(t *testing.T) {
	...
	b3Payload := make([]byte, 1<<18)
	_, err := rand.Read(b3Payload)
	if err != nil {
		t.Fatal(err)
	}
	b3 := Binary(b3Payload)

	payloads := []Payload{&b1, &b2, &s1, &b3}
...
func TestMaxPayload(t *testing.T) {
	...
	if err != ErrMaxPayloadSizeExceeded {
		t.Fatalf("expected ErrMaxPayloadSizeExceeded; actual: %v", err)
	}
}
